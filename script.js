...
function processWeatherData(data, locationName) {
    ...
    if (iconUrl) {
        ...
        weatherIcon.src = getWeatherIconEmoji(description);
        weatherIcon.alt = description;
    }
...
function getWeatherIconEmoji(description) {
    ...
    return createEmojiSVG('🌤️');
}

// Create SVG data URL with emoji
function createEmojiSVG(emoji) {
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
        <text x="50" y="70" font-size="80" text-anchor="middle">${emoji}</text>
    </svg>`;
